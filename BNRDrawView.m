...
{
    // Let's put a log stated to see the order of events
    NSLog(@"%@",NSStringFromSelector(_cmd));
    for (UITouch *t in touches) {
        NSValue *key = [NSValue valueWithNonretainedObject:t];
        BNRLine *line = self.linesInProgress[key];
...
- (void)tap: (UIGestureRecognizer *)gr
{
    ...
    if(self.selectedLine)
    {
        // Make ourselves the target of menu itme action messages
...
{
    //Remove the selecte line from the list of finished lines
...
- (void)moveLine: (UIPanGestureRecognizer *)gr
{
    ...
    // When the pan recognizer changes its postion...
